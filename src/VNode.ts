...
export class VNode {
    ...
    ApplyAttrDiff(newAttrs: { name: string; value: any }[]) {
        let res = Diff<{ name: string; value: any }>(
            this.attrs,
            newAttrs,
            VNode.compairAttr
        );
    ...
    private static compairAttr(
        left: { name: string; value: any },
        right: { name: string; value: any }
    ): boolean {
        ...
        if (this.type == 'standard') {
            if (this.tag != 'fragment') {
                ...
                this.attrs.forEach(attr => {
                    ...
                    } else ApplyAttr(elem, attr.name, attr.value);
                });
                this.children.forEach(child => {
                    let doms = child.ToDom();
