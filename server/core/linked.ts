...
class Procedure {
    ...
    constructor(
        path: string,
        method: HttpMethod,
        middleware: Middleware | Linked,
        fully: boolean,
        attachedLinked: Linked
    ) {
        this.path = path;
        this.method = method;
    ...
    Do(req: IncomingMessage, res: ServerResponse) {
        if (this.match(req, res)) {
            ...
            if (this.next) this.next.Do(req, res);
            else this.attachedLinked.OtherWise(req, res);
        }
    }
    ...
    private match(req: IncomingMessage, res: ServerResponse) {
        ...
        if (this.method != req.method && this.method != 'ALL') return false;
    ...
    $GetContextPath(): string {
        if (this.path.startsWith('/')) return this.path;
        let contextPath = this.attachedLinked.$GetContextPath();
        if (contextPath.endsWith('/')) return contextPath + this.path;
        else return contextPath + '/' + this.path;
    }
}
export class Linked {
    ...
    Use() {
        ...
        if (
            typeof first == 'string' &&
            (typeof second == 'function' || second instanceof Linked)
        ) {
            third = !!third;
        ...
        if (
            ...
            (typeof third == 'function' || third instanceof Linked)
        ) {
    ...
    $GetContextPath(): string {
        if (!this.attachedProcedure) return '';
    ...
    OtherWise(req: IncomingMessage, res: ServerResponse) {
        if (this.otherwise) this.otherwise.Do(req, res);
        else {
            res.statusCode = 404;
